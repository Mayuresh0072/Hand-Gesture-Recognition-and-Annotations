...
cv2.namedWindow('Annotation', cv2.WINDOW_AUTOSIZE)
    ...
    for i in range(len(points)):
        for j in range(len(points[i])):
            for k in range(1, len(points[i][j])):
                ...
                cv2.line(frame, points[i][j][k - 1], points[i][j][k], colors[i], thick)
                cv2.line(paintWindow, points[i][j][k - 1], points[i][j][k], colors[i], thick)
